"""
Offline AI Code Generator — Main entry point (Streaming Version).
...
def handle_project_command(description: str, generator: CodeGenerator, project_gen: ProjectGenerator):
...
def main():
    ...
    while True:
        ...
        # ── Normal code generation (STREAMED) ──────────────
        console.print("[dim]Thinking …[/dim]")
        response_text = ""

        try:
            # Use streaming generator
            for chunk in generator.stream_inference(user_input):
                print(chunk, end="", flush=True)  # Print progressively in terminal
                response_text += chunk
        except KeyboardInterrupt:
            console.print("\n[red]Generation interrupted by user.[/red]")

        # Store conversation cleanly
        generator.conversation_history.append({"role": "user", "content": user_input})
        generator.conversation_history.append({"role": "assistant", "content": response_text})
        rag.add_conversation(user_input, response_text)

        # Print Markdown if code blocks exist
        if "```" in response_text:
            console.print("\n")
